...
export default {
    ...
    methods: {
        ...
        formatDay(today) {
            ...
            let mm = String(newDate.getMonth() + 1).padStart(1, '0'); //January is 1!
            let year = newDate.getFullYear();
            let dayOfWeek = newDate.getDay(); // Sunday is 0, Monday is 1, and so on.
            let dayKind = '';
            let holiday = JSON.parse(this.nationalHoliday);
            let holiyear = holiday[year];
            let holimonth = holiyear[mm];
            if (dayOfWeek == 0) {
                dayKind = '日';
            }
            if (dayOfWeek == 1) {
