...
class TranslationsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $this->_dataTypesTranslations();
        $this->_categoriesTranslations();
        $this->_pagesTranslations();
        $this->_menusTranslations();
    }
     ...
     * @return void
     */
    private function _categoriesTranslations()
    {
        // Adding translations for 'categories'
        //
        $cat = Category::where('slug', 'category-1')->firstOrFail();
        if ($cat->exists) {
            $this->_trans('pt', $this->_arr(['categories', 'slug'], $cat->id), 'categoria-1');
            $this->_trans('pt', $this->_arr(['categories', 'name'], $cat->id), 'Categoria 1');
        }
        $cat = Category::where('slug', 'category-2')->firstOrFail();
        if ($cat->exists) {
            $this->_trans('pt', $this->_arr(['categories', 'slug'], $cat->id), 'categoria-2');
            $this->_trans('pt', $this->_arr(['categories', 'name'], $cat->id), 'Categoria 2');
        }
    }
     ...
     * @return void
     */
    private function _dataTypesTranslations()
    {
     ...
     * @return void
     */
    private function _pagesTranslations()
    {
    ...
    private function _menusTranslations()
    {
        ...
        if ($_item->exists) {
            $this->_trans('pt', $this->_arr($_tpl, $_item->id), 'Funções');
        }

        $_item = $this->findMenuItem(__('voyager::seeders.menu_items.tools'));
        if ($_item->exists) {
            $this->_trans('pt', $this->_arr($_tpl, $_item->id), 'Ferramentas');
        }

        $_item = $this->findMenuItem(__('voyager::seeders.menu_items.menu_builder'));
        if ($_item->exists) {
            $this->_trans('pt', $this->_arr($_tpl, $_item->id), 'Menus');
        }

        $_item = $this->findMenuItem(__('voyager::seeders.menu_items.database'));
        if ($_item->exists) {
            $this->_trans('pt', $this->_arr($_tpl, $_item->id), 'Base de dados');
        }

        $_item = $this->findMenuItem(__('voyager::seeders.menu_items.settings'));
        if ($_item->exists) {
            $this->_trans('pt', $this->_arr($_tpl, $_item->id), 'Configurações');
        }
    }

    private function _findMenuItem($title)
    {
        return MenuItem::where('title', $title)->firstOrFail();
    }

    private function _arr($par, $id)
    {
        return [
            'table_name'  => $par[0],
            'column_name' => $par[1],
    ...
    private function _trans($lang, $keys, $value)
    {
